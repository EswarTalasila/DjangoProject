...
export type AssignmentTemplateActionBarProps = {
  mode: 'create' | 'edit';
...
export default function AssignmentTemplateActionBar({
  mode,
  isSubmitting,
  onCancel,
  isQuickRubricOpen,
...
}: AssignmentTemplateActionBarProps) {
  return (
    <>
      <div className="flex items-center gap-3">
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          {mode === 'create' ? 'Create Assignment Template' : 'Save Changes'}
        </Button>
