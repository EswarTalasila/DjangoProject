...
type AssignmentTemplateBuilderFormProps = {
  mode: 'create' | 'edit';
  assignmentTemplateId?: number;
};

export default function AssignmentTemplateBuilderForm({
  mode,
  assignmentTemplateId,
}: AssignmentTemplateBuilderFormProps) {
  ...
  useEffect(() => {
    ...
    const stored = storage.getItem('assignmentTemplateBuilderShowTips');
    if (stored != null) {
  ...
  useEffect(() => {
    ...
    storage.setItem('assignmentTemplateBuilderShowTips', showTips ? '1' : '0');
  }, [showTips]);
  ...
  // -- Edit mode: fetch existing assignment template --

  useEffect(() => {
    if (mode !== 'edit' || !assignmentTemplateId) return;

    let cancelled = false;

    async function load() {
      try {
        const a: AssignmentTemplate = await getAssignmentTemplate(assignmentTemplateId!);
        if (cancelled) return;
        ...
        if (!cancelled) {
          toast.error('Failed to load assignment template');
  ...
  }, [mode, assignmentTemplateId, router]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload: AssignmentTemplateInput = {
      title: title.trim(),
    ...
    try {
      if (mode === 'create') {
        ...
        router.push(`/dashboard/assignment-templates/${created.id}`);
      } else {
        await updateAssignmentTemplate(assignmentTemplateId!, payload);
        toast.success('Assignment template updated');
        router.replace(`/dashboard/assignment-templates/${assignmentTemplateId}`);
        router.refresh();
      }
    } catch (err: unknown) {
  ...
  function handleCancel() {
    if (mode === 'edit' && assignmentTemplateId) {
      router.push(`/dashboard/assignment-templates/${assignmentTemplateId}`);
    } else {
      router.push('/dashboard/assignment-templates');
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="rounded-sm border border-border bg-card p-6 space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-lg font-semibold text-foreground">Assignment Template Details</h2>
          <Button
            type="button"
            variant="outline"
            size="sm"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="space-y-2 lg:col-span-2">
            <div className="flex items-center gap-1.5">
              ...
              <HelpTip text="Clear name for this assignment template (for example: Week 2 Reading Check)." />
            </div>
            <Input
              id="title"
              placeholder="Enter assignment template title..."
              value={title}
              onChange={(e) => {
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-1.5">
            ...
            <HelpTip text="Optional tag to organize assignment templates. Click + to add or change it, or pick from your existing category bank." />
          </div>
