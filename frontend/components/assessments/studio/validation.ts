...
export function buildStudioValidationIssues({
  ...
  if (gradingMode === 'AUTO' && assessmentRubricId != null) {
    issues.push({
      id: 'assessment-rubric-auto',
      level: 'error',
      panel: 'settings',
  ...
  if (assessmentRubricId != null && hasSpecificRubrics) {
    issues.push({
      id: 'assessment-rubric-mixed-specific-rubrics',
      level: 'error',
      panel: 'settings',
      section: 'rubricBinding',
      title: 'Rubric Configuration',
      detail:
        'Use either an assessment rubric or question/group rubrics. Clear one before using the other.',
    });
  }
  ...
  for (let index = 0; index < questions.length; index += 1) {
    ...
    if (gradingMode === 'MANUAL' && !hasRubric) {
      issues.push({
        ...
        questionIndex: index,
        title: `Question ${index + 1}`,
        detail: 'MANUAL mode requires a rubric at the question, group, or assessment level.',
      });
    }
