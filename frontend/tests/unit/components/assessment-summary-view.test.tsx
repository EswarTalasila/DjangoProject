...
describe("VizCourseSummaryView (assessment-summary-view)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("renders assessment categories", async () => {
    mockFetchCourseSummary.mockResolvedValueOnce(courseSummaryData);
    const Component = await loadComponent();
    render(<Component courseId={1} role="TEACHER" />);
  ...
  it("shows '-' for null assessmentCategory", async () => {
    mockFetchCourseSummary.mockResolvedValueOnce({
      ...courseSummaryData,
      assignments: [
        {
