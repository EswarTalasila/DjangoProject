...
| AUTH | AUTH-UC-05 | POST | `/api/v1/auth/reset-requests` | None | RESEARCHER, TEACHER | Creates `PENDING` request | requester identity fields | AUTH-UC-05-E1/E2/E3 | AUTH-CN-06, AUTH-CN-10 | `test_AUTH_UC_05_*` | Proposed |
| AUTH | AUTH-UC-06 | POST | `/api/v1/auth/reset-request-lookups` | None | RESEARCHER, TEACHER | Reads request status | identifier + request token | AUTH-UC-06-E1/E2 | AUTH-CN-10, AUTH-CN-12 | `test_AUTH_UC_06_*` | Proposed |
| AUTH | AUTH-UC-07 | PATCH | `/api/v1/auth/reset-requests/{id}` | Approver | ADMIN, RESEARCHER | `PENDING -> APPROVED|DENIED` | status + optional expires_at/reason | AUTH-UC-07-E1 | AUTH-CN-06 | `test_AUTH_UC_07_*` | Proposed |
| AUTH | AUTH-UC-07-TEACHER | POST | `/api/v1/courses/{course_id}/students/{student_user_id}/reset-code` | Teacher | TEACHER | Direct student reset code issuance | none (path scoped) | AUTH-UC-07-E1 | AUTH-CN-05, AUTH-CN-06 | `test_AUTH_UC_05_STUDENT` | Proposed |
| AUTH | AUTH-UC-05 | POST | `/api/v1/auth/reset-code-validations` | None | RESEARCHER, TEACHER, STUDENT | N/A | reset code + requester fields | AUTH-UC-05-E4 | AUTH-CN-07 | `test_AUTH_UC_05_*` | Proposed |
| AUTH | AUTH-UC-05 | POST | `/api/v1/auth/password-resets` | None | RESEARCHER, TEACHER, STUDENT | Consumes reset code | reset code + new password + confirm | AUTH-UC-05-E5 | AUTH-CN-07, AUTH-CN-11 | `test_AUTH_UC_05_*` | Proposed |
...
| REG | REG-UC-01 | POST | `/api/v1/registration/accounts` | None | RESEARCHER, TEACHER, STUDENT | N/A | method (LOCAL) + code + name + password (+ username + email for non-student flows) | REG-UC-01-E2/E3 | REG-CN-03, REG-CN-13, REG-CN-22 | `test_REG_UC_01_*` | Proposed |
| REG | REG-UC-01 | POST | `/api/v1/registration/accounts` | None | RESEARCHER, TEACHER | N/A | method (OAUTH) + code + oauth payload + in-app name | REG-UC-01-E4 | REG-CN-03, REG-CN-13, REG-CN-22 | `test_REG_UC_01_*` | Proposed |
...
- `POST /api/v1/registration/accounts` now supports non-student local registration for invite flows (requires `username` + `email` in non-student paths).
...
- AUTH: `login` → `sessions`, `oauth/google` → `sessions/oauth`, `refresh` → `token-exchanges`, `logout` → `session-revocations`, `password/change` → `password` (PATCH), `reset-requests/status` → `reset-request-lookups`, `reset-codes/verify` → `reset-code-validations`, `reset-codes/complete` → `password-resets`.
