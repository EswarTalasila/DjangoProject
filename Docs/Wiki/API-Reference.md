...
|---|---|---|---|---|---|---|---|---|---|---|---|
| AUTH | AUTH-UC-01 | POST | `/api/v1/auth/login` | None | ALL | N/A | identifier + password | AUTH-UC-01-E1/E2/E3 | AUTH-CN-01, AUTH-CN-03, AUTH-CN-04, AUTH-CN-12 | `test_AUTH_UC_01_*` | Proposed |
...
| AUTH | AUTH-UC-06 | POST | `/api/v1/auth/reset-requests/status` | None | RESEARCHER, TEACHER | Reads request status | identifier + request token | AUTH-UC-06-E1/E2 | AUTH-CN-10, AUTH-CN-12 | `test_AUTH_UC_06_*` | Proposed |
...
| REG | REG-UC-01 | POST | `/api/v1/registration/validate-code` | None | RESEARCHER, TEACHER, STUDENT | N/A | access code | REG-UC-01-E1 | REG-CN-01, REG-CN-02, REG-CN-22 | `test_REG_UC_01_*` | Proposed |
| REG | REG-UC-01 | POST | `/api/v1/registration/local` | None | RESEARCHER, TEACHER, STUDENT | N/A | code + name + password (+ username + email for non-student flows) | REG-UC-01-E2/E3 | REG-CN-03, REG-CN-13, REG-CN-22 | `test_REG_UC_01_*` | Proposed |
| REG | REG-UC-01 | POST | `/api/v1/registration/oauth` | None | RESEARCHER, TEACHER, STUDENT | N/A | code + oauth payload + in-app name | REG-UC-01-E4 | REG-CN-03, REG-CN-13, REG-CN-22 | `test_REG_UC_01_*` | Proposed |
...
- `POST /api/v1/registration/local` now supports non-student local registration for invite flows (requires `username` + `email` in non-student paths).
