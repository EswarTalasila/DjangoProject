"""Pure unit tests for assignment_template serializer validation (no database)."""
...
from assignment_templates.serializers import (
...
class TestAssignmentTemplateSerializer:
    """Tests for AssignmentTemplateSerializer validation."""

    def test_valid_assessment(self):
        """Accepts valid assignment_template payload."""
        data = {
            "title": "Quiz 1",
        ...
        s = AssignmentTemplateSerializer(data=data)
        assert s.is_valid(), s.errors

    def test_rejects_missing_title(self):
        """Rejects assignment_template without title."""
        s = AssignmentTemplateSerializer(data={"gradingMode": GradingMode.AUTO})
        assert not s.is_valid()
        assert "title" in s.errors

    def test_accepts_blank_title(self):
        """Allows blank titles for in-progress drafts."""
        s = AssignmentTemplateSerializer(data={"title": "", "gradingMode": GradingMode.AUTO})
        assert s.is_valid(), s.errors

    def test_rejects_missing_grading_mode(self):
        """Rejects assignment_template without gradingMode."""
        s = AssignmentTemplateSerializer(data={"title": "Quiz"})
        assert not s.is_valid()
        assert "gradingMode" in s.errors

    def test_rejects_invalid_grading_mode(self):
        """Rejects invalid grading mode value."""
        s = AssignmentTemplateSerializer(
    ...
    def test_id_is_optional(self):
        """id field is optional (for new assignment_templates)."""
    ...
    def test_accepts_rubric_id_field(self):
        """rubricId is now a real field for assignment_template-level rubrics."""
        data = {
            "title": "Quiz",
            "gradingMode": GradingMode.AUTO,
            "rubricId": None,
    ...
    def test_rejects_legacy_rubric_assessment_ids_field(self):
        """Rejects legacy rubricAssignmentTemplateIds field with validation error."""
        data = {
            "title": "Quiz",
            "gradingMode": GradingMode.AUTO,
