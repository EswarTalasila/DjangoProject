"""Unit tests for assignment_templates.image_views."""
...
class TestFindQuestionByStorageKey:
    @patch("assignment_templates.image_views.parse_question_image")
    @patch("assignment_templates.image_views.Question")
    def test_returns_exact_storage_key_match(self, mock_question_model, mock_parse):
        ...
        from assignment_templates.image_views import _find_question_by_storage_key
...
class TestCanReadQuestionImage:
    @patch("assignment_templates.image_views.Enrollment")
    @patch("assignment_templates.image_views.Assignment")
    def test_student_can_read_via_enrolled_assignment(
        ...
        from assignment_templates.image_views import _can_read_question_image

        user = _user(role=Role.STUDENT)
        question = SimpleNamespace(assignment_template_id=11)
    ...
    @patch("assignment_templates.image_views.can_view_course", return_value=True)
    @patch("assignment_templates.image_views.Assignment")
...
class TestUploadOrDeleteView:
    @patch("assignment_templates.image_views._assessment_is_locked", return_value=True)
    @patch("assignment_templates.image_views.AssignmentTemplate")
    def test_returns_409_when_assignment_template_is_locked(
        self,
        mock_assignment_template_model,
        mock_locked,
    ):
        """Returns 409 when assignment_template is referenced by assignments."""
        from assignment_templates.image_views import upload_or_delete
        ...
        request = _authed_request(
            "post",
            "/api/v1/assignment-templates/1/questions/2/image",
            user=_user(role=Role.RESEARCHER),
        )

        response = upload_or_delete(request, assignment_template_id=1, question_id=2)

        assert response.status_code == status.HTTP_409_CONFLICT
        assert response.data["detail"] == "Cannot modify assignment template referenced by assignments"


class TestServeImageView:
    @patch("assignment_templates.image_views._can_read_question_image", return_value=False)
    @patch("assignment_templates.image_views._find_question_by_storage_key")
    def test_returns_403_when_request_user_cannot_read_image(
        ...
        from assignment_templates.image_views import serve_image

        mock_find_question.return_value = SimpleNamespace(id=5)
        request = _authed_request(
            "get",
            "/api/v1/assignment-templates/images/demo-key",
            user=_user(role=Role.STUDENT),
        )
    ...
    @patch("assignment_templates.image_views.get_storage_backend")
    @patch("assignment_templates.image_views.parse_question_image")
    @patch("assignment_templates.image_views._can_read_question_image", return_value=True)
    @patch("assignment_templates.image_views._find_question_by_storage_key")
