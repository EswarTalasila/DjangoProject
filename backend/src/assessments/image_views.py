...
def _assessment_is_locked(assessment: Assessment) -> bool:
    """Return True when assignments reference the assessment."""
    return Assignment.objects.filter(assessment=assessment).exists()
...
# ---------------------------------------------------------------------------
# POST / DELETE  /api/v1/assessments/{assessment_id}/questions/{question_id}/image
# ---------------------------------------------------------------------------


@api_view(["POST", "DELETE"])
@permission_classes([IsResearcherOrAdmin])
def upload_or_delete(request, assessment_id: int, question_id: int):
    ...
    if _assessment_is_locked(assessment):
        return Response(
            {"detail": "Cannot modify assessment referenced by assignments"},
...
# ---------------------------------------------------------------------------
# GET  /api/v1/assessments/images/<path:storage_key>
# ---------------------------------------------------------------------------
